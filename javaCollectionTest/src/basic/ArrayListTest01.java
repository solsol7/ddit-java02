...
public class ArrayListTest01 {
	...
	public static void main(String[] args) {
		...
		//						  없으면 -1을 반환한다.
		//  -indexOf()메서드는 검색 방향이 앞에서 뒤쪽 방향으로 검색하고
		//   lastIndexOf() 메서드는 뒤에서 앞쪽 방향으로 검색한다.
		//  -리스트에 '비교객체'와 같은 데이터가 여러개면 첫번째로 찾아진 데이터의 위치값(index값)을 반환한다.
		list2.clear();
		list2.add("AAAA");
		list2.add("BBBB");
		list2.add("CCCC");
		...
		System.out.println("ZZZZ의 위치값 : "+list2.lastIndexOf("ZZZZ"));//찾는 데이터가 없으면 -1
		...
		//String[] strArr = (String[])list2.toArray();	//배열 - 여러개의 데이터가 한꺼번에 들어가는 틀 : 배열 자체를 형변환 할 수는 없음
