...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		//2. mapper에 등록된 SQL문 증 실행할 SQL문을 호출해서 원하는 작업을 수행한다.
		/*
		//2-1. insert 연습
		...
		//작성된 SQL을 호출해서 실행하여 결과를 받아오는 객체변수 선언
		SqlSession session = null;
		
		try {
			// 2) SqlSessionFactory객체를 이용하여 SqlSession객체를 구한다.
			//		==> openSession()메서드를 사용한다.
			//	형식) SqlSessionFactory객체.openSession(논리값)
			//		'논리값'이 true이면 AutoCommit이 활성화되고
			//		'논리값'이 생략되거나 false이면 AutoCommit이 비활성화 된다.
			session = sqlSessionFactory.openSession();
			
			//3) SqlSession객체를 이용하여 처리할 SQL문을 호출하여 실행한다.
			//	형식) Sqlsession객체.insert("namespace속성값.실행할id속성값",파라미터클래스)
			//		==> 반환값 : 작업에 성공한 레코드 수 
		...
		*/
		...
		try {
			...
			// 작업을 자동으로 수행한다.
			// 형식) SqlSession객체.selectList("namespace속성값.실행할id속성값", 파라미터클래스)
