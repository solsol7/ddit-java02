...
public class TcpMultiChatClient {
	...
	public void clientStart() {
		Socket socket = null;
		try {
			socket = new Socket("192.168.35.78",7777);
	...
	class ClientSender extends Thread{
		...
		public ClientSender(Socket socket) {
			...
			try {
				...
				if(dout!=null) {
					while(true) {
						// 클라이언트용 프로그램은 처음 실행하면 서버에 접속하고 접속에 성공하면 첫번째로 '대화명'을 입력받아 전송하고,
						// '대화명'의 중복 여부를 응답으로 받아서 확인한다.
		...
		public void run() {
			try {
				while(dout !=null) {
					...
					dout.writeUTF("[" + this.name + "] " + sc.nextLine());
				}
			} catch (Exception e) {
				// TODO: handle exception
			}
