...
public class FileUpload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//GET방식으로 요청하면 FileUpload 폼이 있는 jsp 문서로 포워딩 한다.
		request.getRequestDispatcher("/fileupload/fileUpload.jsp")
			.forward(request, response);
	}

	// 파일 업로드 작업은 POST방식으로 요청해야만 가능하기 때문에 doPost()메서드에 작업한다.
	...
	//------------------------------------------------
	// 파일이 아닌 일반 데이터는 getParameter()메서드나 getParameterValue()메서드를 이용해서 구한다.
	...
	for(Part part : request.getParts()) {
		...
		if(!"".equals(fileName)) {	//파일인지 검사
			...
			try {
				// 파일을 저장한다. ==> Part객체의 write()메서드를 이용한다.
